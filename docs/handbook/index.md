...
| [6. 團隊是怎麼編制的？](06-org-structure.md) | 誰向誰匯報、爭議怎麼升級、Product Manager 這一層在做什麼。 |
| [7. 團隊有哪些固定 workflow？](07-workflows.md) | 八條固定跑法總覽：每條的觸發條件與權威章節。含模型分層、升級規則與供應商路由。 |
...
2. **流程自己會跑**：需求（Product Analyst）→ 設計（Tech Lead）→ 拆單（Product Manager）→ 實作（Developer）→ 審查（Code Reviewer）→ 測試（QA）。每一棒的交接物與放行條件見[第 3 章](03-workflow.md)。
