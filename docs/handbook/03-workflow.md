...
還有**第五步，但它不是結案條件**：版本化的精裝站要不要跟著發一版，由你決定——打 `handbook-v<a>.<b>.<c>.<d>` tag 就會觸發 CI 建站（四碼版本號怎麼進位見[第 7 章](07-workflows.md)第 6 節）。tag 發佈屬你專屬，agent 不會自己打（[第 4 章](04-decision-points.md)）。**推 tag 前不必再解鎖**——擋 `handbook-v*` 的那條 repo 規則已於 2026-09-08 依你的選擇永久關閉，代價（誤推保護沒了）見[第 4 章](04-decision-points.md)。
