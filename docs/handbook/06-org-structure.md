...
那一處例外是 Frontend Verifier 的「可否指派工單」：平台上這個旗標解出來是**開的**，而規範沒有給它派工權。之所以留著，是因為關掉它的那支 API 是單向門——開過再關並不會回到原狀，而能真正清乾淨的設定點只有你（在面板上）動得了。實際影響是零：這個旗標本來就不是平台層的閘門（誰能派工是規則層的自律邊界），所以規範說了不算數的事，平台開著也不會有人去做。**你在 MYL-79 決定不處理它，這是知情下的保留，不是漏同步。** 其餘不一致仍依上一節的兩個方向處理。
