...
Paperclip 上各 agent 的 `reportsTo`（匯報對象）設定是上面組織圖的映射。Frontend Verifier 的匯報線本來就是 CEO，這次是規範追上它。**PM 曾經只存在於規範裡，2026-09-06 已在平台上建置完成**——九個角色**除下述一處刻意保留的例外外，其餘逐格一致**，是逐格核對過的（讀平台的實際權限與掛載，不是看設定面板的開關，那兩者會給出相反的答案）。
