...
| L33 | 拿 `apply_profile.py --smoke`（或任何一次性探針）確認「供應商收不收這組 model×effort」，判準只取 **exit code** | **兩家的「不收」響度不同，取 exit code 只擋得住其中一家。**（a）**codex ⇒ 會炸**：非法 effort 由**伺服器**回 400 `[ReasoningEffortParam] [reasoning.effort] [invalid_enum_value]`、rc 1，CLI 自己對 effort 零驗證（`L31`）。（b）**claude ⇒ 靜默降級**：未知的 `--effort` 值**不是錯誤**——CLI 印一行 `Warning: Unknown --effort value '…' — ignoring it and using the default effort. Valid values: low, medium, high, xhigh, max.` 到 **stderr**，然後**沿用預設 effort 照常跑完、rc 0、stdout 是正常回覆**。⇒ 只看 rc 的探針對 claude 的值域錯誤**必報綠燈**，而且是「跑起來了、跑的卻是別的 effort」這種最不會被發現的形態。⚠️ 危險的不是漏抓，是**印出 ✅**：沉默至少留得住疑心，一個綠勾會讓人停止查。（2026-09-08 MYL-136 第 1 輪 CR 發現、第 2 輪覆驗） | **claude 側必須加驗輸出，不能只讀 rc**；判準寫成「rc≠0 **或** 輸出命中該家的『我沒收這個值』樣態 ⇒ 沒收下」。樣態逐家登記在 `tools/model-routing/probe_providers.py` 的 `smoke_silent_reject`（claude ＝ `("Unknown --effort value",)`；codex ＝ `()`，**空 tuple 表示實證過沒有這回事，未實證要填 `None` 讓工具停下**），`apply_profile.smoke_verdict()` 消費它。⚠️ 這條**比 rc 弱**：警告字樣是會隨版本改的字串，改了比對就靜默失效、那組又變回假綠燈——所以工具逐組印出偵測強度，不對兩家做同一句宣稱，任何複述都不要寫成「smoke 三種失敗都看得到」。⇒ 要**零成本**重取這份樣本：`claude -p --model <非法 model> --effort <非法 effort> <提示詞>`——effort 警告是 CLI 端印的、在請求送出之前，非法 model 讓請求根本送不出去，**不花額度**就看得到那一行。 |
| L34 | 不持 `tasks:assign` grant 的 agent 把工單指派給 **CEO 或 Product Manager**：`PATCH /api/issues/{id}` 送 `{"assigneeAgentId":"<CEO 或 PM 的 id>"}` | **403 `deny_policy_restricted`**，body 逐字為 `{"error":"Target agent is protected and requires an explicit assignment grant.","details":{"reason":"deny_policy_restricted"}}`。這是 **MYL-118（X4）把 CEO／PM 兩格套上 `permissions.authorizationPolicy.assignmentPolicy.mode = "protected"` 的直接後果**；套用前同一條路徑是通的，`L23` 那次 Tech Lead 指派得動是因為**目標格的 policy 還是 `null`**（走 `kind: "none"` 的舊路徑），兩條不衝突，別拿 `L23` 反駁本條。判定鏈（`@paperclipai/server/dist/services/authorization.js`，平台實跑的 `2026.831.1` 樹，行號 2026-09-08 逐條覆核無誤）：`:1595` agent actor 的 `tasks:assign` 分支 → `:1610` `assignmentPolicyEffect()`（定義在 `:855`）→ `mode: "protected"` 在 `:133` 映成 `kind: "restricted"` → `:1614` 走 `:1615` `decideWithTaskAssignmentGrants("agent", actorAgentId)`（`:1007`，先查 `tasks:assign`、再退一步查 `tasks:assign_scope`）→ 兩者皆無 ⇒ `:1618` `denyRestrictedAssignmentPolicy()`（`:1119`）。🔑 **判定讀的是「被指派對象」那一格的 policy，不是 actor 的**——actor 只被拿去查 grant。⚠️ **而且目標格不只一個**：`:858`–`:877` 同時評估 **target agent（`:860`）／target project（`:863`）／target issue（`:866`）／parent issue（`:869`）** 四個來源，取 `unknown` > `blocked` > `restricted` > `none` 的優先序 ⇒ 看到 `deny_policy_restricted` **不能直接認定是 agent 那一格**。**分辨方法在訊息開頭那個 label**：四個來源各自帶 `Target agent`／`Target project`／`Target issue`／`Parent issue` 前綴（`:79` `evaluateAuthorizationPolicyForAssignment(policy, label)`），本次實測回的是 **`Target agent`**，才確定是 agent 格 | **替代路徑＝在單上留言。** 實測：Tech Lead（`grants: []`）在 **assignee 為 CEO（`protected`）的 MYL-118** 上 `POST …/comments` → **201**，回應的 `authorizationReason` 是 **`allow_direct_parent_report`**。⚠️ **這條路比「公司內 agent 都能留言」窄**，別照後者宣稱：`:1424`–`:1432` 的條件是 `trustResolution.kind === "standard"` ＋ action 為 `issue:comment` ＋ **目標單是本 run 來源單的直屬母單**（MYL-118 正是 MYL-138 的母單）。留言給**非母單**的第三張單走的是另一組判準（`:1628`–`:1657`：assignee 是自己／該單無 agent assignee／有 mention grant／`decideVisibleIssueWrite`），**本輪未量測**。⇒ 需要持 grant 的人接手時，把「誰接手做什麼」寫成留言由 CEO／PM 去指派。⚠️ **取證一定要配對照組**：只有「403」這一個觀測，與「這條路由根本不通」**觀測相同**，分不出擋你的是 policy 還是別的東西。本次對照組＝同一 actor／同一 endpoint／同一張單，只把 `assigneeAgentId` 換成**沒有 policy** 的 QA（`14732ae9`）⇒ **200 且實際寫入**，才證明擋下來的是 policy；另對 CEO 再打一發得到逐字相同的 403，排除「PM 那一格特有」。（探針用 `done` 單——`done` 不起 run；測完立即還原，同 `L23` 的紀律。）⚠️ **200 那一發驗不出走的是哪條放行路徑**：`kind: "none"` 底下有兩條（`:1620` `trustResolution.kind === "standard"` ⇒ `decideVisibleIssueWrite()`，否則才是 `allow_simple_company_member`），而 `PATCH /api/issues/{id}` 的回應**不帶任何 authorization reason 欄位**（實查回應全部 59 個頂層鍵，無 `authorizationReason`／`authorizationTrace`）⇒ 別宣稱閘門套用前走的是哪一條。🔴 **`access.canAssignTasks: true` 不等於持有 grant，別拿它預判這道閘門**：2026-09-08 逐個打 `GET /api/agents/{id}` 掃過全公司 9 名，`Frontend Verifier` 是 `canAssignTasks: true`／`taskAssignSource: "simple_default"` 但 **`grants: []`**（正是 `L23`／`L26` 那個開過又關、回不去的 `simple_default` 落點），而 `:1615` 查的是**真的 grant**。真正持有 `tasks:assign` 的**全公司只有 CEO（`ceo_role` 自帶）與 Product Manager（`explicit_grant`）兩名**，`mode: "protected"` 的也正好是這兩格。🔑 **機制：`simple_default` 是衍生值不是儲存值。** `buildAgentAccessState()`（`routes/agents.js:768`）的優先序是 `ceo_role` → `agent_creator`（讀的是 `permissions.canCreateAgents`）→ `explicit_grant`（有真 grant）→ **`simple_default`（只要 membership 是 `active`）** → `none`，**全程不讀 `permissions.canAssignTasks`**（`grep -n canAssignTasks services/authorization.js` 命中 **0** ⇒ 那個旗標對本閘門完全沒有作用——FV 正是 `permissions.canAssignTasks: false` 卻 `access.canAssignTasks: true`，**同一份回應裡兩欄互相矛盾**）。而 `setPrincipalPermission()`（`services/access.js:377`）在 `!enabled` 時**只刪掉那一列 grant 就 `return`、從不刪 membership**，`enabled` 時則會 `ensureMembership` ⇒ **「開過又關」必然落在 `membership: active` ＋ `grants: []` ＝ `simple_default`**。⇒ 這給 `L23`／`L26` 的「回不去」一個**精確射程：回不去的是那個衍生顯示值，能力本身撤得掉**——`:1615` 查的就是被刪掉的那一列。也因此 **FV 結構上必然過不了 `:1615`**（`decidePrincipalGrant()` 先查 membership（FV 有）再查 grant（FV 無）⇒ `deny_missing_grant`）。⚠️ **但這一段是原始碼推論、不是量測**——只有非持有者本人打得出那一發，本輪 actor 是 Tech Lead。⚠️ **grants 只讀得到單筆端點**：`GET /api/companies/{cid}/agents`（列表）**整個沒有 `access` 這個鍵**（實查頂層 25 鍵），照它盤點會得到「全員 `grants: []`」的假象，要盤 grant 只能逐個 id 打 `GET /api/agents/{id}`。（2026-09-08 MYL-138 實測收容，承接 MYL-118 AC4——該單當輪只有 CEO 的金鑰、做不出非持有者身分，故只到原始碼推論並誠實記在留言 `1f7a0d15`，本條把它補成量測；量測結論與當時的推論一致） |
| L35 | 要讓一個 agent 退場，於是按面板 agent 動作選單裡的 **`Leave agent`**；或在文件／交接裡把退場動作寫成 `pause` ＋ `leave` | **`Leave agent` 不是退場，它一個欄位都不動 agent。** 它打的是 `PUT /api/companies/{cid}/resource-memberships/me/agents/{agentId}`（路由 `@paperclipai/server/dist/routes/resource-memberships.js:75`；UI bundle 的 `$Q.updateAgent` 綁的就是這個 URL），把**呼叫者自己**（`requireBoardUserId(req, res)` 取的 board user）在 `agentMemberships` 那一列的 `state` 設成 `left`。**寫入目標可以逐句數**：服務層 `services/resource-memberships.js` 的 `updateAgent`（`:219`）只讀 `agents` 表做存在性驗證（`:220`–`:224`），而全檔僅有的四個寫入語句是 `:188` `projectMemberships`／`:263` `agentMemberships`／`:327` `documentMemberships`／`:371` `documentMemberships`——**沒有一個寫 `agents`**。⇒ 按完之後 agent 照樣在名冊上、照樣被喚醒、照樣計費，只是**你自己的側邊欄看不到它**。真正的退場在同一組動作選單的 **`Terminate`**（UI 呼叫 `Xt.terminate(agentId)`，另有 `hideTerminate`／`onTerminateSuccess` 一組）。⚠️ **而 `terminate` 不可逆**（`services/agents.js`）：`terminate`（`:714`）把 `status` 設成 `terminated`（`:721`）並把該 agent **所有 API key 設 `revokedAt`**（`:728`–`:731`）；此後四道守衛各自 `throw conflict`——`pause`（`:646` `Cannot pause terminated agent`）、`resume`（`:666` `Cannot resume terminated agent`）、`clearError`（`:689`）、`updateAgent`（`:463` `Terminated agents cannot be resumed`）。名冊也讀不到了：`:572`–`:573` 在沒帶 `includeTerminated` 時把 `terminated` 濾掉 ⇒ `GET /api/companies/{cid}/agents` 少一名，要查只剩單筆 `GET /api/agents/{id}`。**連 `Leave` 都回不去**：membership 的 `updateAgent` 在 `:223` 對 `status === "terminated"` 直接 `throw notFound("Agent not found")` | **按之前先分清楚你要哪一個**：「只從我的側邊欄收起來、日後還想要它回來」→ `Leave agent`；「這個角色從此不存在」→ `Terminate`，**不可逆、屬 `H5` 破壞性操作**，只有使用者按得動且 agent 一律不得代按（本平台 agent 也沒有那把權限）。⚠️ **文件裡不要再把退場寫成 `pause` ＋ `leave`**——MYL-96 的裁定與 MYL-115 的落地都這麼寫，於是 protocol 第 9 節、手冊 `06-org-structure.md` 與 `.foundry/org.yml` **三處同時**把一個不可逆動作描述成「全程可逆」「都可以一鍵還原」，而 MYL-118 為那顆鈕撤過兩次互動卡才發現按了不會有任何事發生。⇒ **描述一個平台動作之前，先確認那顆鈕打的是哪一支 API**，「名字看起來像」不是依據。（2026-09-08 MYL-139 收容；行號取自實跑樹 `2026.831.1`，驗明正身的取證法見 `L32`） |
