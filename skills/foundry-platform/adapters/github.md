...
同一次作業連續建 N 張來源單時，**時機 1 的五步要跟著每一張走完**。這一格有自己的規則，
是因為時機 1 是 `【自律】` 而對帳是延遲偵測：漏掉的那一張當下不會有任何東西提醒你，
要等**下一個人 commit** 才浮出來，而那時已經有好幾個 run 醒著（`X9` 記錄了完整經過：
一次建八張、只鏡像六張，紅燈流出去之後被兩個 run 各補一次，同一張單建出兩張鏡像）。

1. **一張一鏡像。** 來源端 `create_issue` 成功 → 立刻走完時機 1 的五步 → 才建下一張。
   建單迴圈裡**不得有「稍後統一鏡像」這個階段**：回頭補的那份清單來自記憶，
   而漏掉的那一張正好就是記憶裡沒有的那一張。
2. **批次收尾前自查，不要把偵測留給下一個 commit 的人。** 最後一張建完後當場跑
   `python3 tools/foundry-lint/foundry_lint.py --selfcheck`，讀 `mirror-recon` 那一行——
   `⏭` 不算通過（見「對帳」節），拿不到任一端時改用該節的 `gh issue list` 一行指令自己比對。
   自查在本輪就把漏建收掉，紅燈才不會流到別人面前變成競態。
...
對帳是**延遲偵測**，而 run 是併行的——同一片紅燈會被好幾個 run 在同一分鐘裡各自看到。
於是「看到就順手補」這個直覺會把漏建變成一對多，**而送出去的公開 issue 收不回來**。

- **漏建：只補自己建的那張來源單。** 不是你建的**不得代建**；改在該來源工單留一則跨單通報，
  由建立者補。2026-09-06 實際發生：`#37` 與 `#38` 由兩個不同的 run 在 27 秒內各建一張，
  指向同一張來源單（`X9`）。
- **一對多、孤兒：只回報、不動手。** 關掉或改寫公開 issue 屬 `G-C`。
- **動手前先重查一次，不要照著手上那份輸出動手。** 紅燈訊息本身會過期，而且**兩個方向都會**：
  同一次事件裡「漏建」與「一對多」兩則相反的通報都是過期讀取，照任一則動手都會出錯（`X9`）。
- **例外只有一種**：發單者在工單裡**明文授權**代建（例如它自己受規則拘束、當下不得查鏡像端現況）。
  這種授權要寫在工單上，不由執行者自己推定。

**收斂重複鏡像的手法（可逆，優先於刪除）**：移除多餘那張的 `Foundry-Source:` 首行、改寫 body 說明它已作廢，
再 `gh issue close <N> --reason "not planned"`；並回來源工單補一則更正登記留言，把作廢的 `Mirrored-to:` 標明。
移除首行之後對帳就看不到它了（見「對應標記」節：body 首行是唯一權威），**不必刪 issue**——
刪掉收不回來，也留不下「這裡曾經重複過」的痕跡。
