...
**適用對象**：**CEO 與 Product Manager，就這兩個角色**。`F1` 是列舉不是通則，其餘角色不在範圍內（見 protocol `F1` 末句）。

| 邊界 | 可以 | 不可以 |
| --- | --- | --- |
| **等級**（`F1`） | `L1`：開頁、等渲染、截圖 | 一切互動（點擊、填表、導航到要操作才到得了的狀態、故障注入），以及**任何超過 `L1` 的動作**——例如跑 Lighthouse（它是 `L3` 的唯讀稽核，擋它的理由是超過上限，不是因為它算互動） |
...
2. **粒度是「工具」不是「等級」**：tool-profile 切的是「哪些工具可用」。要靠它切出 `F1`，
   得逐工具維護一張放行清單（放行 `take_screenshot`、擋掉 `click`／`fill`／`navigate`…），
   而那張清單會隨 MCP server 改版增減工具而漂移——漏掉一個新增的互動工具就破一個口，
   且沒有任何機制會提醒你它漂了。

⚠️ **不要以為「把整個 server 拿掉」就等於 `F1`**——那條路看似可行，實際上答非所問：
拿掉 MCP 確實會落到 `L1`（§2 的 `L1` 判定訊號只看瀏覽器二進位、不含 MCP；
`probe_browser.py` 的 `compute_level()` 也是無可用 MCP 即回 `1`，仍能渲染＋截圖）。
問題在於 `.mcp.json` 是**情境級**的：拿掉它是把**這個 repo 所有 agent** 的互動能力一起拿掉，
Frontend Verifier 也不例外。那是取消整個 repo 的 `L2`，不是限制某個角色——`F1` 要的是後者。
