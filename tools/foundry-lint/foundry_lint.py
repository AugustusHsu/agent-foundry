...
def git_run(root: Path, *args) -> tuple:
    ...
    """
    # `GIT_*` 要洗掉再跑：pre-commit 執行 hook 時會**匯出 `GIT_DIR`**，而它的
    # 優先序高於 `-C`——於是 `--repo-root` 指到別的 repo（自檢的測試、或在
    # worktree 裡跑）時，這裡會安靜地去讀「當初觸發 hook 的那個 repo」。
    # 平常兩者剛好是同一個，所以這個錯不會表現出來，只在最需要它準的時候出錯。
